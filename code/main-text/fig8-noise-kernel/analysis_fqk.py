...
batch2 = [1,2,3,4,5,6,7]

kernel = np.zeros((len(noise),len(layer_tot),ndata))
kernel2 = np.zeros((len(noise),len(layer_tot),ndata))
...
np.save('fqk_n%i_noise.npy'%qubit, avg_con)
